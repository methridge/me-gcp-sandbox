...
resource "null_resource" "r1-stack-init" {
  provisioner "local-exec" {
    command     = "/Users/methridge/sandboxes/_bin/stack-init.sh"
    working_dir = "${path.module}/r1/"
    environment = {
      LB_IP   = module.region-1-stack.region-lb-ip
      LB_NAME = google_dns_record_set.lb-region-1.name
    }
  }
  provisioner "local-exec" {
    command     = "/Users/methridge/sandboxes/_bin/stack-destroy.sh"
    working_dir = "${path.module}/r1/"
    when        = destroy
  }
...
resource "null_resource" "r2-stack-init" {
  provisioner "local-exec" {
    ...
    environment = {
      ...
      LB_NAME = google_dns_record_set.lb-region-2.name
    }
  }
  provisioner "local-exec" {
    command     = "/Users/methridge/sandboxes/_bin/stack-destroy.sh"
...
resource "null_resource" "r3-stack-init" {
  provisioner "local-exec" {
    ...
    environment = {
      ...
      LB_NAME = google_dns_record_set.lb-region-3.name
    }
  }
  provisioner "local-exec" {
    command     = "/Users/methridge/sandboxes/_bin/stack-destroy.sh"
