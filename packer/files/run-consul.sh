...
function print_usage {
  ...
  echo -e "  --wan-cluster-tag-name\tAutomatically form a WAN cluster with Instances that have the same value for this Compute Instance tag name."
  echo -e "  --enable-acls\tEnable ACLs."
...
function generate_consul_config {
  ...
  local -r wan_cluster_tag_name=${15}
  local -r enable_acls=${16}
  ...
  if [[ -z "$wan_cluster_tag_name" ]]; then
    log_warn "The --wan-cluster-tag-name property is empty. Will not automatically try to form a WAN cluster based on Cluster Tag Name."
  else
    retry_join_wan_key_val="\"retry_join_wan\": [\"provider=gce project_name=$project_id tag_value=$wan_cluster_tag_name\"],"
  fi
  ...
  while [[ $# > 0 ]]; do
    ...
    case "$key" in
      ...
      --cluster-tag-name)
        ...
        shift
        ;;
      --wan-cluster-tag-name)
        wan_cluster_tag_name="$2"
        shift
        ;;
      --raft-protocol)
        assert_not_empty "$key" "$2"
